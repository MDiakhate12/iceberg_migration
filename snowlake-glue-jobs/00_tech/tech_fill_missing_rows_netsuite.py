...
def collect_primary_keys(datasource, start_date=None, end_date=None, date_column=None):
    ...
    max_duration = int(args.get("max-duration", 3600))  # Default to 1 hour if not set
...
end_date = "31/12/2025"
table = args.get("table", "transactionaccountingline")
...
print(f"Found {len(set(primary_keys) - set(target_primary_keys))} primary keys in source but not in target (might be missing)")

# %%
missing_primary_keys = set(primary_keys) - set(target_primary_keys)

number_of_missing_primary_keys = len(missing_primary_keys)
...
if missing_primary_keys:
    ...
    print(f"Fetching {datasource.source_table_alias} source data for {number_of_missing_primary_keys} missing primary keys...")

    if is_running_locally:
        max_workers = min(30, number_of_missing_primary_keys)
    else:
        max_workers = min(100, number_of_missing_primary_keys)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for pk_tuple in missing_primary_keys:
    ...
    print("No missing primary keys to fetch data for.")
    results = []
...
if results:
    ...
    etl_tools.write_json(results, tmp_json_path)
